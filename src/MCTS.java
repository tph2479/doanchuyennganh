...
//		System.out.println("Cases calculated: " + evaluationCount + " Calculation time: " + (System.currentTimeMillis() - startTime) + " ms");
	...
	private Object[] MCTSSearch(boolean isFirst) {
		...
		rootNode.getState().setPlayerType(moveCount == 1 ? PlayerType.PLAYER : PlayerType.AL);
		// root là người chơi đánh
		// nếu ai đánh trước thì board là người chơi
...
//        bestMove[2] = winnerNode.getState().getBoard().getLastMove()[1];
	...
	private void backPropogation(Node nodeToExplore, int playoutResult) {
		...
		while (tempNode != null) {
			...
			if (tempNode.getState().getPlayerType() == (aiStarts ? PlayerType.AL : PlayerType.PLAYER)) {
				if (aiStarts && playoutResult == 1) {
					tempNode.getState().addWinScore(1);
				} else if (!aiStarts && playoutResult == 2) {
					tempNode.getState().addWinScore(1);
				}
			}
...
//		System.out.println();
	...
	private Object[] searchWinningMove(Board board) {
		...
		for (int[] move : allPossibleMoves) {
			...
			// move.
			if (Game.checkWinner(dummyBoard) != 0) {
