...
public class Minimax {
	...
	// This variable is used to track the number of evaluations for benchmarking
	// purposes.
	...
	// This function calculates the relative score of the white player against the
	// black.
	...
	// (i.e. How good a player's general standing on the board by considering how
	// many
	...
	public int[] calculateNextMove(int depth) {
		...
		System.out.println("Cases calculated: " + evaluationCount + " Calculation time: "
	...
	/*
	 * alpha : Best AI Move (Max) beta : Best Player Move (Min) returns: {score,
	 * move[0], move[1]}
	 */
	private static Object[] MCTSSearchAB(int depth, Board dummyBoard, boolean max, double alpha, double beta) {
		...
		/*
		 * (Move 1) / (Current Node) --- (Move 2) \ ... (Move N)
		 */
		ArrayList<int[]> allPossibleMoves = dummyBoard.generateMoves();

		// If there is no possible move left, treat this node as a terminal node and
		// return the score.
		...
		if (max) {
			...
			for (int[] move : allPossibleMoves) {
				...
				// (if the depth > 0) and searches for the minimum white score in each of the
				// sub trees.
				// We will find the maximum score of this depth, among the minimum scores found
				// in the
				// lower depth.
				...
				// When searching for the minimum, if the score of a node is lower than the
				// alpha
				...
				// Pruning with beta
				// Beta value holds the minimum score among the uncle nodes from one upper
				// level.
				// We need to find a score lower than this beta score, because any score higher
				// than
				// beta will be eliminated by the minimizing player (upper level). If the score
				// is
				// higher than (or equal to) beta, break out of loop discarding any remaining
				// nodes
			...
			// Initialize the starting best move using the first move in the list and
			// +infinity score.
			...
			for (int[] move : allPossibleMoves) {
				...
				// This function recursively generates new MCTS trees branching from this node
				// (if the depth > 0) and searches for the maximum white score in each of the
				// sub trees.
				// We will find the minimum score of this depth, among the maximum scores found
				// in the
				// lower depth.
				...
				// When searching for the maximum, if the score of a node is higher than the
				// beta
				...
				// Pruning with alpha
				// Alpha value holds the maximum score among the uncle nodes from one upper
				// level.
				// We need to find a score higher than this alpha score, because any score lower
				// than
				// alpha will be eliminated by the maximizing player (upper level). If the score
				// is
				// lower than (or equal to) alpha, break out of loop discarding any remaining
				// nodes
				// and/or subtrees and return the last move.
				if ((Double) (tempMove[0]) <= alpha) {
					return tempMove;
	...
	private static Object[] searchWinningMove(Board board) {
		...
		for (int[] move : allPossibleMoves) {
			...
			// If the white player has a winning score in that temporary board, return the
			// move.
	...
	public static int evaluateHorizontal(int[][] boardMatrix, boolean forBlack, boolean playersTurn) {
		...
		// blocks variable is used to check if a consecutive stone set is blocked by the
		// opponent or
		// the board border. If the both sides of a consecutive set is blocked, blocks
		// variable will be 2
		// If only a single side is blocked, blocks variable will be 1, and if both
		// sides of the consecutive
		...
		// If there is another empty cell after a consecutive stones set, block count
		// will again be
		// decremented by 1.
	...
	public static void evaluateDirections(int[][] boardMatrix, int i, int j, boolean isBot, boolean botsTurn,
			int[] eval) {
		// Check if the selected player has a stone in the current cell
		...
		// Check if there were any consecutive stones before this empty cell
			...
			// Reset consecutive stone count
			eval[0] = 0;
			// Current cell is occupied by opponent, next consecutive set may have 2 blocked
			// sides
			eval[1] = 2;
		} else {
			// Current cell is occupied by opponent, next consecutive set may have 2 blocked
	...
	private static void evaluateDirectionsAfterOnePass(int[] eval, boolean isBot, boolean playersTurn) {
		// End of row, check if there were any consecutive stones before we reached
		// right border
		if (eval[0] > 0) {
	...
	// blocks: Number of blocked sides of the set (2: both sides blocked, 1: single
	// side blocked, 0: both sides free)
	public static int getConsecutiveSetScore(int count, int blocks, boolean currentTurn) {
		...
		case 4: {
			...
			if (currentTurn)
				...
				// Opponent's turn
				// If neither side is blocked, 4 consecutive stones guarantees a win in the next
				// turn.
				if (blocks == 0)
					return winGuarantee / 4;
				// If only a single side is blocked, 4 consecutive stones limits the opponents
				// move
				// (Opponent can only place a stone that will block the remaining side,
		...
		case 3: {
			...
			if (blocks == 0) {
				...
				// (User places another stone to make the set 4 consecutive, opponent can only
				// block one side)
				// However the opponent may win the game in the next turn therefore this score
				// is lower than win
				// guaranteed scores but still a very high score.
				if (currentTurn)
					return 50_000;
				// If it's the opponent's turn, this set forces opponent to block one of the
				// sides of the set.
